// <auto-generated />
using System;
using LiberDent.Data.Data;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

namespace LiberDent.Data.Migrations
{
    [DbContext(typeof(LiberDentContext))]
    partial class LiberDentContextModelSnapshot : ModelSnapshot
    {
        protected override void BuildModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "3.1.3")
                .HasAnnotation("Relational:MaxIdentifierLength", 128)
                .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

            modelBuilder.Entity("LiberDent.Data.Data.Account.AdresUzytkownika", b =>
                {
                    b.Property<int>("IdAdresu")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("IdUzytkownika")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("KodPocztowy")
                        .IsRequired()
                        .HasColumnType("nvarchar(7)")
                        .HasMaxLength(7);

                    b.Property<string>("Miejscowosc")
                        .IsRequired()
                        .HasColumnType("nvarchar(50)")
                        .HasMaxLength(50);

                    b.Property<string>("NumerDomu")
                        .IsRequired()
                        .HasColumnType("nvarchar(15)")
                        .HasMaxLength(15);

                    b.Property<string>("NumerLokalu")
                        .HasColumnType("nvarchar(15)")
                        .HasMaxLength(15);

                    b.Property<string>("Poczta")
                        .IsRequired()
                        .HasColumnType("nvarchar(50)")
                        .HasMaxLength(50);

                    b.Property<string>("Ulica")
                        .IsRequired()
                        .HasColumnType("nvarchar(50)")
                        .HasMaxLength(50);

                    b.HasKey("IdAdresu");

                    b.HasIndex("IdUzytkownika")
                        .IsUnique()
                        .HasFilter("[IdUzytkownika] IS NOT NULL");

                    b.ToTable("AdresUzytkownika");
                });

            modelBuilder.Entity("LiberDent.Data.Data.Account.ApplicationUser", b =>
                {
                    b.Property<string>("Id")
                        .HasColumnType("nvarchar(450)");

                    b.Property<int>("AccessFailedCount")
                        .HasColumnType("int");

                    b.Property<string>("ConcurrencyStamp")
                        .IsConcurrencyToken()
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("CzyLekarz")
                        .HasColumnType("bit");

                    b.Property<string>("Email")
                        .HasColumnType("nvarchar(256)")
                        .HasMaxLength(256);

                    b.Property<bool>("EmailConfirmed")
                        .HasColumnType("bit");

                    b.Property<bool>("LockoutEnabled")
                        .HasColumnType("bit");

                    b.Property<DateTimeOffset?>("LockoutEnd")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("NormalizedEmail")
                        .HasColumnType("nvarchar(256)")
                        .HasMaxLength(256);

                    b.Property<string>("NormalizedUserName")
                        .HasColumnType("nvarchar(256)")
                        .HasMaxLength(256);

                    b.Property<string>("PasswordHash")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("PhoneNumber")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("PhoneNumberConfirmed")
                        .HasColumnType("bit");

                    b.Property<string>("SecurityStamp")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("TwoFactorEnabled")
                        .HasColumnType("bit");

                    b.Property<string>("UserName")
                        .HasColumnType("nvarchar(256)")
                        .HasMaxLength(256);

                    b.HasKey("Id");

                    b.HasIndex("NormalizedEmail")
                        .HasName("EmailIndex");

                    b.HasIndex("NormalizedUserName")
                        .IsUnique()
                        .HasName("UserNameIndex")
                        .HasFilter("[NormalizedUserName] IS NOT NULL");

                    b.ToTable("AspNetUsers");
                });

            modelBuilder.Entity("LiberDent.Data.Data.Account.DaneLekarza", b =>
                {
                    b.Property<int>("IdDaneLekarza")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<int>("IdPoziomUprawnien")
                        .HasColumnType("int");

                    b.Property<int>("IdStanowiska")
                        .HasColumnType("int");

                    b.Property<int>("IdTytulNaukowy")
                        .HasColumnType("int");

                    b.Property<string>("IdUzytkownika")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("Opis")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("PoziomUprawnienIdPoziomUprawnien")
                        .HasColumnType("int");

                    b.Property<int?>("StanowiskaIdStanowiska")
                        .HasColumnType("int");

                    b.Property<int?>("TytulNaukowyIdTytulNaukowy")
                        .HasColumnType("int");

                    b.HasKey("IdDaneLekarza");

                    b.HasIndex("IdUzytkownika")
                        .IsUnique()
                        .HasFilter("[IdUzytkownika] IS NOT NULL");

                    b.HasIndex("PoziomUprawnienIdPoziomUprawnien");

                    b.HasIndex("StanowiskaIdStanowiska");

                    b.HasIndex("TytulNaukowyIdTytulNaukowy");

                    b.ToTable("DaneLekarza");
                });

            modelBuilder.Entity("LiberDent.Data.Data.Account.DaneUzytkownika", b =>
                {
                    b.Property<int>("IdDaneUzytkownika")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<DateTime>("DataUrodzenia")
                        .HasColumnType("datetime2");

                    b.Property<string>("IdUzytkownika")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("Imie")
                        .IsRequired()
                        .HasColumnType("nvarchar(50)")
                        .HasMaxLength(50);

                    b.Property<string>("Nazwisko")
                        .IsRequired()
                        .HasColumnType("nvarchar(70)")
                        .HasMaxLength(70);

                    b.Property<string>("NumerPESEL")
                        .IsRequired()
                        .HasColumnType("nvarchar(11)")
                        .HasMaxLength(11);

                    b.HasKey("IdDaneUzytkownika");

                    b.HasIndex("IdUzytkownika")
                        .IsUnique()
                        .HasFilter("[IdUzytkownika] IS NOT NULL");

                    b.ToTable("DaneUzytkownika");
                });

            modelBuilder.Entity("LiberDent.Data.Data.CMS.CenyUslug", b =>
                {
                    b.Property<int>("IdCenyUslug")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<decimal>("CenaUslugi")
                        .HasColumnType("decimal(18,2)");

                    b.Property<string>("NazwaUslugi")
                        .IsRequired()
                        .HasColumnType("nvarchar(100)")
                        .HasMaxLength(100);

                    b.HasKey("IdCenyUslug");

                    b.ToTable("CenyUslug");
                });

            modelBuilder.Entity("LiberDent.Data.Data.CMS.DaneOPersonelu", b =>
                {
                    b.Property<int>("IdDaneOPersonelu")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<int>("IdStanowiska")
                        .HasColumnType("int");

                    b.Property<int>("IdTytulNaukowy")
                        .HasColumnType("int");

                    b.Property<string>("Imie")
                        .IsRequired()
                        .HasColumnType("nvarchar(30)")
                        .HasMaxLength(30);

                    b.Property<string>("Nazwisko")
                        .IsRequired()
                        .HasColumnType("nvarchar(30)")
                        .HasMaxLength(30);

                    b.Property<string>("Opis")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("StanowiskaIdStanowiska")
                        .HasColumnType("int");

                    b.Property<int?>("TytulNaukowyIdTytulNaukowy")
                        .HasColumnType("int");

                    b.Property<string>("Zdjecie")
                        .HasColumnType("nvarchar(256)")
                        .HasMaxLength(256);

                    b.HasKey("IdDaneOPersonelu");

                    b.HasIndex("StanowiskaIdStanowiska");

                    b.HasIndex("TytulNaukowyIdTytulNaukowy");

                    b.ToTable("DaneOPersonelu");
                });

            modelBuilder.Entity("LiberDent.Data.Data.CMS.GodzinyOtwarcia", b =>
                {
                    b.Property<int>("IdGodzinyOtwarcia")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<bool>("CzyAktywny")
                        .HasColumnType("bit");

                    b.Property<string>("Dzien")
                        .IsRequired()
                        .HasColumnType("nvarchar(20)")
                        .HasMaxLength(20);

                    b.Property<string>("GodzinaOtwarciaDo")
                        .IsRequired()
                        .HasColumnType("nvarchar(12)")
                        .HasMaxLength(12);

                    b.Property<string>("GodzinaOtwarciaOd")
                        .IsRequired()
                        .HasColumnType("nvarchar(12)")
                        .HasMaxLength(12);

                    b.Property<int>("PozycjaWyswietlania")
                        .HasColumnType("int");

                    b.HasKey("IdGodzinyOtwarcia");

                    b.ToTable("GodzinyOtwarcia");
                });

            modelBuilder.Entity("LiberDent.Data.Data.CMS.Kontakt", b =>
                {
                    b.Property<int>("IdKontaktu")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<bool>("CzyAktywny")
                        .HasColumnType("bit");

                    b.Property<string>("NazwaTelefonu")
                        .IsRequired()
                        .HasColumnType("nvarchar(30)")
                        .HasMaxLength(30);

                    b.Property<string>("NumerTelefonu")
                        .IsRequired()
                        .HasColumnType("nvarchar(12)")
                        .HasMaxLength(12);

                    b.Property<int>("PozycjaWyswietlania")
                        .HasColumnType("int");

                    b.Property<string>("TrescKontaktu")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("TytułKontakt")
                        .HasColumnType("nvarchar(20)")
                        .HasMaxLength(20);

                    b.HasKey("IdKontaktu");

                    b.ToTable("Kontakt");
                });

            modelBuilder.Entity("LiberDent.Data.Data.CMS.ONas", b =>
                {
                    b.Property<int>("IdONas")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Naglowek")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Tresc1")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Tresc2")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("IdONas");

                    b.ToTable("ONas");
                });

            modelBuilder.Entity("LiberDent.Data.Data.CMS.Powitanie", b =>
                {
                    b.Property<int>("IdPowitania")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<bool>("CzyAktywny")
                        .HasColumnType("bit");

                    b.Property<string>("NaglowekPowitalny")
                        .IsRequired()
                        .HasColumnType("nvarchar(150)")
                        .HasMaxLength(150);

                    b.Property<string>("TrescPowitalna1")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("TrescPowitalna2")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("IdPowitania");

                    b.ToTable("Powitanie");
                });

            modelBuilder.Entity("LiberDent.Data.Data.CMS.UmowioneWizyty", b =>
                {
                    b.Property<int>("IdUmowioneWizyty")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("ApplicationUserId")
                        .HasColumnType("nvarchar(450)");

                    b.Property<bool>("CzyPacjentByl")
                        .HasColumnType("bit");

                    b.Property<int?>("DaneLekarzaIdDaneLekarza")
                        .HasColumnType("int");

                    b.Property<DateTime>("DataWizyty")
                        .HasColumnType("datetime2");

                    b.Property<TimeSpan>("GodzinaWizyty")
                        .HasColumnType("time");

                    b.Property<string>("IdApplicationUser")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("IdLekarza")
                        .HasColumnType("int");

                    b.Property<int>("IdRodzajeWizyty")
                        .HasColumnType("int");

                    b.Property<int?>("RodzajeWizytyIdRodzajeWizyty")
                        .HasColumnType("int");

                    b.HasKey("IdUmowioneWizyty");

                    b.HasIndex("ApplicationUserId");

                    b.HasIndex("DaneLekarzaIdDaneLekarza");

                    b.HasIndex("RodzajeWizytyIdRodzajeWizyty");

                    b.ToTable("UmowioneWizyty");
                });

            modelBuilder.Entity("LiberDent.Data.Data.Slownikowe.PoziomUprawnien", b =>
                {
                    b.Property<int>("IdPoziomUprawnien")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<int>("NumerUprawnien")
                        .HasColumnType("int");

                    b.Property<string>("OpisUprawnien")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("IdPoziomUprawnien");

                    b.ToTable("PoziomUprawnien");
                });

            modelBuilder.Entity("LiberDent.Data.Data.Slownikowe.RodzajeWizyty", b =>
                {
                    b.Property<int>("IdRodzajeWizyty")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Nazwa")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("IdRodzajeWizyty");

                    b.ToTable("RodzajeWizyty");
                });

            modelBuilder.Entity("LiberDent.Data.Data.Slownikowe.Stanowiska", b =>
                {
                    b.Property<int>("IdStanowiska")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<bool>("CzyStomatolog")
                        .HasColumnType("bit");

                    b.Property<string>("Nazwa")
                        .IsRequired()
                        .HasColumnType("nvarchar(40)")
                        .HasMaxLength(40);

                    b.HasKey("IdStanowiska");

                    b.ToTable("Stanowiska");
                });

            modelBuilder.Entity("LiberDent.Data.Data.Slownikowe.TytulNaukowy", b =>
                {
                    b.Property<int>("IdTytulNaukowy")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Nazwa")
                        .IsRequired()
                        .HasColumnType("nvarchar(50)")
                        .HasMaxLength(50);

                    b.Property<string>("Skrot")
                        .IsRequired()
                        .HasColumnType("nvarchar(11)")
                        .HasMaxLength(11);

                    b.HasKey("IdTytulNaukowy");

                    b.ToTable("TytulNaukowy");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRole", b =>
                {
                    b.Property<string>("Id")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("ConcurrencyStamp")
                        .IsConcurrencyToken()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(256)")
                        .HasMaxLength(256);

                    b.Property<string>("NormalizedName")
                        .HasColumnType("nvarchar(256)")
                        .HasMaxLength(256);

                    b.HasKey("Id");

                    b.HasIndex("NormalizedName")
                        .IsUnique()
                        .HasName("RoleNameIndex")
                        .HasFilter("[NormalizedName] IS NOT NULL");

                    b.ToTable("AspNetRoles");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRoleClaim<string>", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("ClaimType")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ClaimValue")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("RoleId")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("Id");

                    b.HasIndex("RoleId");

                    b.ToTable("AspNetRoleClaims");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserClaim<string>", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("ClaimType")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ClaimValue")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("UserId")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("Id");

                    b.HasIndex("UserId");

                    b.ToTable("AspNetUserClaims");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserLogin<string>", b =>
                {
                    b.Property<string>("LoginProvider")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("ProviderKey")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("ProviderDisplayName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("UserId")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("LoginProvider", "ProviderKey");

                    b.HasIndex("UserId");

                    b.ToTable("AspNetUserLogins");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserRole<string>", b =>
                {
                    b.Property<string>("UserId")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("RoleId")
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("UserId", "RoleId");

                    b.HasIndex("RoleId");

                    b.ToTable("AspNetUserRoles");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserToken<string>", b =>
                {
                    b.Property<string>("UserId")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("LoginProvider")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("Value")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("UserId", "LoginProvider", "Name");

                    b.ToTable("AspNetUserTokens");
                });

            modelBuilder.Entity("LiberDent.Data.Data.Account.AdresUzytkownika", b =>
                {
                    b.HasOne("LiberDent.Data.Data.Account.ApplicationUser", "Uzytkownik")
                        .WithOne("Adres")
                        .HasForeignKey("LiberDent.Data.Data.Account.AdresUzytkownika", "IdUzytkownika");
                });

            modelBuilder.Entity("LiberDent.Data.Data.Account.DaneLekarza", b =>
                {
                    b.HasOne("LiberDent.Data.Data.Account.ApplicationUser", "Uzytkownik")
                        .WithOne("DaneLekarza")
                        .HasForeignKey("LiberDent.Data.Data.Account.DaneLekarza", "IdUzytkownika");

                    b.HasOne("LiberDent.Data.Data.Slownikowe.PoziomUprawnien", "PoziomUprawnien")
                        .WithMany("DaneLekarza")
                        .HasForeignKey("PoziomUprawnienIdPoziomUprawnien");

                    b.HasOne("LiberDent.Data.Data.Slownikowe.Stanowiska", "Stanowiska")
                        .WithMany()
                        .HasForeignKey("StanowiskaIdStanowiska");

                    b.HasOne("LiberDent.Data.Data.Slownikowe.TytulNaukowy", "TytulNaukowy")
                        .WithMany()
                        .HasForeignKey("TytulNaukowyIdTytulNaukowy");
                });

            modelBuilder.Entity("LiberDent.Data.Data.Account.DaneUzytkownika", b =>
                {
                    b.HasOne("LiberDent.Data.Data.Account.ApplicationUser", "Uzytkownik")
                        .WithOne("Dane")
                        .HasForeignKey("LiberDent.Data.Data.Account.DaneUzytkownika", "IdUzytkownika");
                });

            modelBuilder.Entity("LiberDent.Data.Data.CMS.DaneOPersonelu", b =>
                {
                    b.HasOne("LiberDent.Data.Data.Slownikowe.Stanowiska", "Stanowiska")
                        .WithMany("DaneOPersonelu")
                        .HasForeignKey("StanowiskaIdStanowiska");

                    b.HasOne("LiberDent.Data.Data.Slownikowe.TytulNaukowy", "TytulNaukowy")
                        .WithMany("DaneOPersonelu")
                        .HasForeignKey("TytulNaukowyIdTytulNaukowy");
                });

            modelBuilder.Entity("LiberDent.Data.Data.CMS.UmowioneWizyty", b =>
                {
                    b.HasOne("LiberDent.Data.Data.Account.ApplicationUser", "ApplicationUser")
                        .WithMany("UmowioneWizyty")
                        .HasForeignKey("ApplicationUserId");

                    b.HasOne("LiberDent.Data.Data.Account.DaneLekarza", "DaneLekarza")
                        .WithMany("UmowioneWizyty")
                        .HasForeignKey("DaneLekarzaIdDaneLekarza");

                    b.HasOne("LiberDent.Data.Data.Slownikowe.RodzajeWizyty", "RodzajeWizyty")
                        .WithMany("UmowioneWizyty")
                        .HasForeignKey("RodzajeWizytyIdRodzajeWizyty");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRoleClaim<string>", b =>
                {
                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityRole", null)
                        .WithMany()
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserClaim<string>", b =>
                {
                    b.HasOne("LiberDent.Data.Data.Account.ApplicationUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserLogin<string>", b =>
                {
                    b.HasOne("LiberDent.Data.Data.Account.ApplicationUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserRole<string>", b =>
                {
                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityRole", null)
                        .WithMany()
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("LiberDent.Data.Data.Account.ApplicationUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserToken<string>", b =>
                {
                    b.HasOne("LiberDent.Data.Data.Account.ApplicationUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });
#pragma warning restore 612, 618
        }
    }
}
