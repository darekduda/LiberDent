...
using LiberDent.PortalWWW.Extensions;
using LiberDent.PortalWWW.Models.ManageViewModel;
...
namespace LiberDent.PortalWWW.Controllers
{
    ...
    public class ManageController : DbController
    {
        ...
        public async Task<IActionResult> MojeWizyty()
        {
            ...
            {
                UmowioneWizyty = await db.UmowioneWizyty.Where(c=>c.ApplicationUser==user).Include(a=>a.DaneLekarza).Include(c=>c.DaneLekarza.TytulNaukowy).Include(b=>b.DaneLekarza.Uzytkownik).ThenInclude(c=>c.Dane).Include(c=>c.RodzajeWizyty).ToListAsync()
            };
            return View(model);
        }
    }
